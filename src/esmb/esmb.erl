%%% ====================================================================
%%% Modified by Dmitry Kirillov for ejabberd NTLM authorization.
%%% 15 Jul 2009
%%%
%%% Added function:
%%% user_logon(S, Neg, U, LM_hash, NTLM_hash) 
%%% c_user_logon(S, Neg, U, LM_hash, NTLM_hash) 
%%% smb_session_setup_andx_pdu(Neg, U, LM_hash, NTLM_hash) 
%%%
%%% Modified code for use iconv(from ejabberd) 
%%%
%%% original code:
%%% --------------------------------------------------------------------
%%% File    : esmb.erl
...
%%% @end
%%%
%%% $Id: esmb.erl,v 1.24 2005/11/24 13:59:10 etnt Exp $
%%% --------------------------------------------------------------------
-module(esmb).
-export([ucase/1, lcase/1, check_dir/3, bin2ip/1,
         connect/1, connect/2, connect/3, close/1, error_p/1,
         user_logon/3, user_logon/4, user_logon/5, emsg/3, emsg/1, get_user_groups/4,
...
read_file(S, InReq, Finfo, ?STREAM_READ, Acc) when ?READ_ENOUGH(Finfo) ->
    Data = trim_binary(list_to_binary(lists:reverse(Acc)), 
...
%%% @end
%%%
%% ------------ Added by Dmitry Kirillov, 2009 -------------------------    
user_logon(S, Neg, U, LM_hash, NTLM_hash) ->
    catch c_user_logon(S, Neg, U, LM_hash, NTLM_hash).
%% ------------ Added by Dmitry Kirillov, 2009 -------------------------    
...
%% ------------ Added by Dmitry Kirillov, 2009 -------------------------    
c_user_logon(S, Neg, U, LM_hash, NTLM_hash) ->
    {Req, Pdu} = smb_session_setup_andx_pdu(Neg, U, LM_hash, NTLM_hash),
...
dec_session_setup_andx(Req, Pdu) -> 
    ...
    Res#smbpdu{samba2 = true}.

is_samba2(Res) ->
    case ejabberd_regexp:run(b2l(Res#smbpdu.bf), "Samba 2") of
        match -> Res#smbpdu{samba2 = true};
        _     -> Res#smbpdu{samba2 = false}
    end.

is_netware(Res) ->
    case ejabberd_regexp:run(b2l(Res#smbpdu.bf), "NetWare") of
        match -> Res#smbpdu{netware = true};
...
x2_get_more(Req, Data) ->
    ...
    if 
        (TotDataCount == (DataCount + DataDisplacement)) ->
            ...
            Res#smbpdu{data = list_to_binary([Data,NewData])};
        (TotDataCount > (DataCount + DataDisplacement)) ->
            ?sdbg("x2_get_more - Need more ~n", [TotDataCount - (DataCount + DataDisplacement)]),
            x2_get_more(Res, list_to_binary([Data,NewData]));
        true ->
...
%%%
%% ------------ Added by Dmitry Kirillov, 2009 -------------------------    
smb_session_setup_andx_pdu(Neg, U, LM_hash, NTLM_hash) when ?PRE_DOS_LANMAN_2_1(Neg) ->
    Passwd   = LM_hash,
    PwLen    = sizeof(LM_hash),
    MacKey   = <<>>,
    ...
    enc_smb(Rec);
%%%
smb_session_setup_andx_pdu(Neg, U, LM_hash, NTLM_hash) when ?NTLM_0_12(Neg),
                                        ?USE_UNICODE(Neg) ->
    RN     = NTLM_hash,
    RNlen  = sizeof(NTLM_hash),
    MacKey = <<>>,
...
%% ------------ Modified by Dmitry Kirillov, 2009 -------------------------    
...
%%    iconv:close(Cd),
    ...
    Uname = iconv:convert(?CSET_ASCII, ?CSET_UCS2LE, U#user.name),
    Udom = iconv:convert(?CSET_ASCII, ?CSET_UCS2LE, U#user.primary_domain),
    Unos = iconv:convert(?CSET_ASCII, ?CSET_UCS2LE, U#user.native_os),
    Ulan = iconv:convert(?CSET_ASCII, ?CSET_UCS2LE, U#user.native_lanman),
%% ------------ Modified by Dmitry Kirillov, 2009 -------------------------    
...
%%    iconv:close(Cd),
    ...
    UCS2pw = iconv:convert(Cset, ?CSET_UCS2LE, Passwd),
%% ------------ Modified by Dmitry Kirillov, 2009 -------------------------    

    EncKey = Neg#smb_negotiate_res.encryption_key,
    RN = nt_challenge_response(UCS2pw, EncKey),
    {ok, X} = crypto:hash(md4, UCS2pw),
    {ok, NTsessKey} = crypto:hash(md4, X),
...
%%    iconv:close(Cd),
    UCS2pw = iconv:convert(?CSET_ASCII, ?CSET_UCS2LE, "qwe123"),
%% ------------ Modified by Dmitry Kirillov, 2009 -------------------------    
...
%%    {ok, UCS2pw} = iconv:conv(Cd, l2b(Passwd)),
%%    iconv:close(Cd),
    UCS2pw = iconv:convert("ASCII", ?CSET_UCS2LE, Passwd),
%% ------------ Modified by Dmitry Kirillov, 2009 -------------------------    

    RN = nt_challenge_response(UCS2pw, EncKey),
    {ok, X} = crypto:hash(md4, UCS2pw),
    {ok, Y} = crypto:hash(md4, X),
    concat(Y, RN).
...
%%    iconv:close(Cd),
    Ustr = iconv:convert(?CSET_ASCII, ?CSET_UCS2LE, Str),
%% ------------ Modified by Dmitry Kirillov, 2009 -------------------------    
    Ustr;
to_ucs2(_, Str) ->    
    l2b(Str).
...
%%            Ustr
%%    end.
    Ustr = iconv:convert(?CSET_UCS2LE, esmb:ucase(Cset), Ustr).
%% ------------ Modified by Dmitry Kirillov, 2009 -------------------------    
